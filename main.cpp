...
void explore(std::ofstream& outstream, double P, double e, double a, double b, double L) {
	...
	std::vector<double> vector_dists;
	...
	for (auto j = 0; j < vector_lyambda.size(); ++j) {
		...
		vector_w.push_back(w_min);
		vector_d.push_back(d_min);
		vector_dists.push_back(dist(min_result, 0));
	}
	...
	print_vector(outstream, vector_w, "w:");
	print_vector(outstream, vector_d, "d:");
	print_vector(outstream, vector_dists, "dists:");
}
...
{
	std::srand(time(0));
	std::ofstream outstream("output_monday.txt");
	// 1) получение x
